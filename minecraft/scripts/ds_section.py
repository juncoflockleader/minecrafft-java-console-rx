#!/usr/bin/env python3
"""Vertical (x-y) cross-section of the Death Star at a fixed Z. Read-only.
Usage: ds_section.py <z> <x0> <x1> <y0> <y1>
Legend: . air  # solid  H ladder  D door  = plate  s stairs  ~ water  L light
Rows are descending Y (top of build at top of output)."""
...
z = int(sys.argv[1]); x0, x1, y0, y1 = map(int, sys.argv[2:6])
cmd(f"forceload add {x0} {z} {x1} {z}")
print(f"  z={z}  x:{x0}..{x1}  y:{y1}..{y0} (top→bottom)")
